...
// ---------- Pure CSS Swipe-to-delete (no nested Framer Motion) ----------
...
const SwipeableDelete = ({ children, onDelete, className = "", isOpen = false, disabled = false, onOpenStateChange }: SwipeableDeleteProps) => {
  const [offsetX, setOffsetX] = useState(isOpen ? -100 : 0);
  ...
  const handlePointerMove = (e: React.PointerEvent) => {
    ...
    const next = Math.max(-100, Math.min(0, startOffsetRef.current + delta));
    setOffsetX(next);
  };
  ...
  return (
    <div className={`relative overflow-hidden ${className} bg-bg-base`}>
      ...
      <div className="absolute inset-y-0 right-0 w-28 flex items-center justify-center px-item z-0">
        <button
          ...
          style={{ opacity: Math.max(0, Math.min(1, (-offsetX - 20) / 80)) }}
          className="w-14 h-14 rounded-button bg-semantic-danger text-text-primary flex items-center justify-center active:scale-90 transition-transform ease-apple"
        >
          <Trash2 className="size-icon-lg" />
        </button>
      </div>

      {/* Swipeable content - pure CSS, no nested motion.div */}
      <div
        ...
        style={{
          ...
          transition: startXRef.current !== null ? "none" : "transform 0.3s cubic-bezier(0.25, 0.46, 0.45, 0.94)",
          touchAction: "pan-y",
        }}
        className="relative z-10 bg-surface-primary overflow-hidden rounded-card border border-hairline border-border-subtle"
      >
        {children}
      </div>
    </div>
